...
func mapTx(tx *bc.TxData) (headerID bc.Hash, hdr *Header, entryMap map[bc.Hash]Entry, err error) {
	...
	for i, inp := range tx.Inputs {
		if oldIss, ok := inp.TypedInput.(*bc.IssuanceInput); ok {
			...
			var issID bc.Hash
			issID, _, err = addEntry(newIssuance(nonceRef, val, hashData(inp.ReferenceData), i))
			if err != nil {
	...
	var h Entry
	headerID, h, err = addEntry(newHeader(tx.Version, resultRefs, hashData(tx.ReferenceData), tx.MinTime, tx.MaxTime))
	if err != nil {
